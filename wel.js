...
module.exports = async (Pendragon, anu) => {
  try {
    let metadata = await Pendragon.groupMetadata(anu.id);
    ...
    for (let num of participants) {
      try {
        ppuser = await Pendragon.profilePictureUrl(num, "image");
      } catch {
        ppuser = botImage4;
      }
...
*Thank You.*
  `;
        if (WELstatus) {
          await Pendragon.sendMessage(anu.id, {
            image: { url: ppuser },
            caption: Pendragontext,
            mentions: [num],
          });
        }
